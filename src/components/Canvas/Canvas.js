      ...
      <div className={`w-full flex justify-center dark:bg-lightGray relative`} style={{minHeight:CANVAS_HEIGHT}}>
        ...
        <div className="flex items-end">
          {generatedArray.map((element) => {
            return (
              <div id={`${element}`} font-size={FONT_SIZE} data={`${(BAR_WIDTH > 25 && element > FONT_SIZE+16 && showHeight) ? element : ''}`} length="40px" key={Math.random()} style={{"--bar-font-size" :FONT_SIZE+"px", height: element+'px', width: BAR_WIDTH+'px', backgroundColor: this.props.barColor}} 
